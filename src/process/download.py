...
def download_files(page: ft.Page, run_type: Run_Type, env_type: Env_Type)-> Column:
    ...
    chosen_folder_bar = Row(visible=False, controls=[], alignment=ft.MainAxisAlignment.CENTER)
    ...
    def reset_folder_selection(e):
        ...
        checkboxes.clear()
        file_label.value = TEXTS.CHOOSE_FILES.value
    ...
    def get_files(bucket: str, folder: str) -> list[str]:
        ...
        return [file.split('/').pop() for file in list_folders_and_files if not file.endswith('/') and not file.endswith(':') and file]
    ...
    def update_file_label(file_count: int) -> None:
        ...
        file_label.value = f"Selected {file_count} files." if file_count else TEXTS.CHOOSE_FILES.value
        page.update()
        ...
        try:
            if validate_download():
                show_progress_popup(
                    ...
                    bucket,
                    folder,
                    action_func=lambda bucket, folder, file_path: download_files_from_gcp(page, bucket, folder, file_path)
                )
