...
def setup_ui(page):
    context = {}
    context["login_button"] = ElevatedButton("Sign in with Google", on_click = lambda e: threading.Thread(target=start_gcloud_login, args=(page, context)).start())
    context["status_text"] = ft.Text()
    context["auth_code_field"] = ft.TextField(label="Paste the verification code here", visible=False)
    context["confirm_button"] = ElevatedButton("Confirm verification code", visible=False, on_click = lambda e: handle_auth_code_submission(page, context))
