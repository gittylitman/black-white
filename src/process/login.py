...
def start_login_process(page: ft.Page, column: Column, login_button: ElevatedButton) -> None:
    ...
    page.update()
    threading.Thread(
        target=perform_gcloud_login,
        args=(page, column, login_button),
        daemon=True
    ).start()


def perform_gcloud_login(page: ft.Page, column: Column, login_button: ElevatedButton) -> None:
    ...
    confirm_button = ElevatedButton(
        ...
        on_click=lambda e: handle_auth_code_submission(page, auth_code_field.value, column, login_button)
    )
...
def handle_auth_code_submission(page: ft.Page, code: str, column: Column, login_button: ElevatedButton) -> None:
    ...
    code = code.strip()
    if not code:
        show_message(page, VALIDATION_MESSAGES.MISSING_VERIFICATION_CODE.value, ft.colors.ORANGE)
        return

    if not gcloud_process:
        show_message(page, ERROR_MESSAGES.GCLOUD_PROCESS_NOT_AVAILABLE.value, ft.colors.ORANGE)
        return
...
def _handle_error(page: ft.Page, column: Column, login_button: ElevatedButton, message: str) -> None:
    show_message(page, message, ft.colors.RED)
